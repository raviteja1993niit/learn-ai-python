...
- [ ] `workflow/pr-description-<story-id>.md` exists and contains all nine sections — if absent or
  incomplete, invoke the **`pr-description` sub-agent** (`.claude/agents/pr-description.md`)
  before proceeding
...
- `skill:mcp-github` — create/update PR, link Jira story, query PR status
- `skill:mcp-bitbucket` — create/update PR, link Jira story, query PR status
- `skill:mcp-jenkins` — trigger build, poll pipeline status, fetch build log
- `skill:git-local` — push branch to remote; verify branch sync
- `skill:read-file` — read `commit-history-<story-id>.md`, `pr-description-<story-id>.md`, `workflow/workflow-config.json`
- `skill:write-file` — save `build-log-<story-id>.txt`
...
3. Raise a new PR **or** update an existing one using the content of
   `workflow/pr-description-<story-id>.md` (generated by the **`pr-description` sub-agent**)
   as the PR body. If the file is missing, invoke the sub-agent first.
...
- [ ] `pr-description-<story-id>.md` confirmed present with all nine sections (invoke `pr-description` sub-agent if absent)
