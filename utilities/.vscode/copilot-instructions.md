...
applyTo: '**'
description: >
  Mastercard PGS pgs-acquirer-elavon-interface-service — workspace context, technology stack,
  build commands, environment layout, and key AI-assistance notes for all Copilot interactions.
---

# Workspace Context — pgs-acquirer-elavon-interface-service

## Project Overview

**pgs-acquirer-elavon-interface-service** is a PCF-based (Cloud Foundry) Acquirer Interface Service
for the **Elavon S2A Acquirer**, developed and maintained by Mastercard as part of the PGS (Payment
Gateway Services) Connectivity platform.

This is a **lift-and-shift modernization** project (`107651-pgsaaselavon`) targeting migration from
legacy PCF deployment toward modern infrastructure.
...
- **Environment-specific:** `application-{env}.yml` (dev1, dev2, itf1, itf2, stage1, stage2) with `-green` variants
...
- **Jenkinsfile:** Uses the Mastercard `common-pipeline-library` shared library
...
- **TCP connectivity:** Elavon communication is over TCP/IP — see `elavon/tcp/` package
...
## Agent & Instruction Files (this workspace)

| Location | Purpose |
|----------|---------|
| `.vscode/agents/` | Specialist agents for each SDLC pipeline phase |
| `.vscode/prompts/` | User-facing workflow commands |
| `.vscode/instructions/` | Coding, API, git, security, and testing standards |
| `.vscode/instructions/skills/` | Domain-specific knowledge bases (coding-style, build-analysis, etc.) |
| `.vscode/references/` | Example code review outputs and reference documents |
