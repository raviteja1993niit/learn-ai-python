"""
server.py — FastAPI server for VectorRAG App.
"""
...
from src.rag import VectorRAGPipeline

app = FastAPI(title="VectorRAG App")
...
# ── Session store ─────────────────────────────────────────────────────────────
sessions: dict = {}
# Each entry: {"pipeline": VectorRAGPipeline, "chat_history": []}
...
async def chat(body: dict):
    ...
    pipeline: VectorRAGPipeline = session["pipeline"]
...
async def get_pages(session_id: str, search: Optional[str] = None):
    ...
    pipeline: VectorRAGPipeline = sessions[session_id]["pipeline"]
    stats = pipeline.stats
